...
/**
 * <p>
 * i茅台API调用Service业务层处理
 * </p>
 *
 * @author Jzj
 * @since 2024/9/10 14:33
 */
@Slf4j
@Service
public class MtApiServiceImpl implements MtApiService {
    ...
    public void appointmentResults() {
        log.info("申购结果查询开始=========================");
        List<MtUser> iUsers = mtUserService.selectReservationUser();
        for (MtUser iUser : iUsers) {
            try {
                ...
                for (Object itemVO : itemVOs) {
                    ...
                    if (item.getInteger("status") == 2 && DateUtil.between(item.getDate("reservationTime"), new Date(), DateUnit.HOUR) < 24) {
                        ...
                        mtLogService.reservation(iUser, logContent);
                    }
                }
            } catch (Exception e) {
...
//        data.put("MT-APP-Version", MT_VERSION);
    ...
    public void reservation(MtUser user) {
        ...
        for (String itemId : items) {
            try {
                String shopId = mtShopService.getShopId(user.getShopType(), itemId,
        ...
        //日志记录
        mtLogService.reservation(user, logContent.toString());
        //预约后延迟领取耐力值
    ...
    public void reservationBatch() {
        ...
        List<MtUser> iUsers = mtUserService.selectReservationUserByMinute(minute);
        for (MtUser iUser : iUsers) {
            //异步预约
            AsyncManager.me().execute(AsyncFactory.mtReservation(iUser));
        }
    }
    ...
    public void getTravelRewardBatch() {
        ...
        List<MtUser> iUsers = mtUserService.selectReservationUserByMinute(minute);
        for (MtUser iUser : iUsers) {
            AsyncManager.me().execute(AsyncFactory.mtTravelReward(iUser));
        }
    }
...
//        本月剩余旅行奖励
    ...
    public JSONObject reservation(MtUser iUser, String itemId, String shopId) {
        ...
        map.put("sessionId", mtItemService.getCurrentSessionId());
